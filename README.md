...
**年龄口径（重要）**：简历上的年龄按**真实出生年份 2002（24 岁）**，时间线为 **2020.09 入学、2024.06 毕业**——18 岁入学、22 岁毕业，完全正常，**不需要任何解释**。
身份证登记为 2007.04（户籍晚报，差 5 岁），**这件事不写进简历**；被 HR 问到时按 `interview/` 里的 30 秒口径解释。

`education.verify` 用来挂学信网在线验证码。
...
> ⚠️ **先核对出生日期再决定挂不挂**：备案表上有一栏「出生日期」，它来自学信档案里的身份信息（通常跟着身份证走）。
> **如果那里显示的是 2007，就会和你简历上的「24 岁 · 2002」当场冲突**——那等于用一份官方文件证明你另一个口径是错的。
> **挂上去之前，先自己打开那个报告看一眼出生日期栏**：
> - 显示 **2002** → 可以挂，正好印证真实年龄；
> - 显示 **2007** → **不要挂**，并在面试时主动用 30 秒口径说明身份证登记晚的事。
